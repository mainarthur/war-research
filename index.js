...
const results = entries.map((entry) => {
    ...
    result.content = JSON.parse(entry.response.content.text).features.filter(e => e.properties.fill).map(e => ({ ...e, area: AreaUtils.geometry(e.geometry) / 1000000, fill: e.properties.fill }))
    result.colums = Object.fromEntries(columns.map(name => [name, 0]))
    result.content.forEach(e => {

        e.type = e.fill === "#000000" && e.properties.name === "Під питанням" ? "Идут активные бои/Под вопросом" : colorMap[e.fill]
...
for (let i = 0; i < results.length; i++) {
    ...
    columns.forEach((name, index) => ws.cell(row, index + 2).number(result.colums[name]))
}
